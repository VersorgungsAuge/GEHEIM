...
#plot <H>00
plt.plot(x, E_p_null+E_m_null -L_p(x)-L_m(x) +m*(abl_zeta_p(x))**2+m*(abl_zeta_m(x))**2, "lime", label="$<H>$", linewidth=5)
plt.axhline(y=H_dach_nullnull, color='m', linestyle='-', label="$H_{mittel}$", linewidth=5)
plt.xlabel("$t/s$", fontsize=26)
plt.ylabel("$Energie/J$", fontsize=26)
