...
k=1 #federkonstante der einzelnen oszillatoren x1 x2
k_2=2 #kopplungskonstante zwischen x1 x2
m=1 #masse
...
#plot <p12>_nl
plt.plot(x, (m/np.sqrt(2))*(abl_zeta_p(x) - abl_zeta_m(x)) , "yellowgreen", label="$<p_1>_{nl}(t)$")
plt.plot(x, (m/np.sqrt(2))*(abl_zeta_p(x) + abl_zeta_m(x)), "darkolivegreen", label="$<p_2>_{nl}(t)$")
plt.xlabel("$t/s$")
plt.ylabel("$Impuls/(Ns)$")
plt.title("Erwartungswert $<p_1>_{nl}(t)$,$<p_2>_{nl}(t)$ \n fuer beliebigen Zustand $\psi_{nl}$")
plt.grid()
plt.legend(loc="best")
plt.tight_layout()
plt.savefig("<p12>nl")
